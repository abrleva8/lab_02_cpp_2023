...
void FileOutput::write_input_data_to_file(std::ofstream& fout, const Board& board) {
	board.print_board(&fout);
}

bool FileOutput::save_input_data_to_file(const Board& board) {
	std::cout << "Please, input the filename:" << std::endl;
	std::string filename;
	...
	if (fout.is_open()) {
		...
		write_input_data_to_file(fout, board);
	} else {
		fout.close();
		return false;
	}
...
bool FileOutput::save_output_data_to_file(Simulator simulator) {
	std::cout << "Please input the filename:" << std::endl;
	std::string filename;
	std::getline(std::cin, filename);
	try_overwrite_file(filename);
	...
	if (fout.is_open()) {
		...
		write_output_data_to_file(fout, std::move(simulator));
	} else {
		fout.close();
		return false;
	}
...
void FileOutput::save_input_data(const Board& board) {
	std::cout << "Do you want to save input data to file? Input please y/n." << std::endl;

	if (ConsoleInput::is_choice_yes()) {
...
bool FileOutput::save_output_data(const Simulator& simulator) {
